...
const DepartmentCRUDComponent = () => {
    ...
    const [sortConfig, setSortConfig] = useState({
        key: 'createdAt', // 'totalStudents', 'year', 'createdAt'
        ...
        try {
            ...
            if (data.success) {
                ...
                setStats({
                    ...
                    // Handle aggregation structure from API: [{_id: null, total: X}]
    ...
    // 1. Extract Unique Departments for Filter Dropdown
    ...
    const processedRecords = useMemo(() => {
        ...
        // A. Filter by Dropdowns
        if (filters.department) {
            result = result.filter(r => r.department === filters.department);
        }
        if (filters.year) {
            result = result.filter(r => getYearLabel(r.year) === filters.year);
        }
    ...
    // CRUD Handlers (Add, Update, Delete) - Same as previous logic
        ...
        if (newRecord.dept && newRecord.div) {
            try {
                ...
                if (data.success) {
                    ...
                    setIsFormOpen(false);
                    // Re-fetch to update aggregate stats
                    fetchClasses();
                ...
                <div className="max-w-7xl mx-auto px-4 py-4">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="grid grid-cols-3 gap-3 text-sm min-w-fit">
                            <div className="bg-blue-50 px-4 py-2 rounded-xl border border-blue-100 flex flex-col items-center justify-center">
                                <Building2 className="text-blue-500 mb-1" size={18} />
                            ...
                            <div className="bg-indigo-50 px-4 py-2 rounded-xl border border-indigo-100 flex flex-col items-center justify-center">
                                <GraduationCap className="text-indigo-500 mb-1" size={18} />
                            ...
                            <div className="bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100 flex flex-col items-center justify-center">
                                <Users className="text-emerald-500 mb-1" size={18} />
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className="p-5 h-full flex flex-col justify-between">
                                            ...
                                            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleEditStart(record)} className="text-xs font-medium text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors">
                                                    Edit
                                                </button>
                                                <button onClick={() => handleDeleteRecord(record._id)} className="text-xs font-medium text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors">
                                                    Remove
                                                </button>
                                            </div>
                                        </div>
